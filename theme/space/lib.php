 ...
 * @return array[]
 */
function theme_space_user_preferences(): array {
    return [
        'drawer-open-block' => [
            'type' => PARAM_BOOL,
...
function theme_space_get_extra_scss($theme) {
    ...
    if (theme_space_get_setting('accessiblelinks') == 1) {
        $content .= '.underline--anim { @extend %underline-anim-accessible; }';
    }
 ...
 * @return bool
 */
function theme_space_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    $theme = theme_config::load('space');
    ...
    foreach ($instances as $instance) {
        ...
        if ($plugin->show_enrolme_link($instance)) {
            $html = html_writer::tag('div', get_string(
                ...
                'theme_space',
                ['url' => $CFG->wwwroot . '/enrol/index.php?id=' . $courseid]
            ));
            break;
        }
    }
